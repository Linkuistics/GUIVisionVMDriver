...
private func handleExec(_ request: Request) async throws -> Response {
    ...
    let stdout = String(data: outData, encoding: .utf8)?.replacingOccurrences(of: "\\s+$", with: "", options: .regularExpression) ?? ""
    let stderr = String(data: errData, encoding: .utf8)?.replacingOccurrences(of: "\\s+$", with: "", options: .regularExpression) ?? ""
