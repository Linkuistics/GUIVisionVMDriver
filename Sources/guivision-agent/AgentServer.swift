...
struct ScreenshotWindowRequest: Decodable {
    let window: String
    let output: String
}

struct ScreenshotElementRequest: Decodable {
    let role: String?
    let label: String?
    let window: String?
    let id: String?
...
struct ScreenshotRegionRequest: Decodable {
    let window: String
    let x: Int
    let y: Int
    let width: Int
...
func buildAgentRouter() -> Router<BasicRequestContext> {
    ...
    router.post("/screenshot-window") { req, _ in try await handleScreenshotWindow(req) }
    router.post("/screenshot-element") { req, _ in try await handleScreenshotElement(req) }
    router.post("/screenshot-region") { req, _ in try await handleScreenshotRegion(req) }
...
private func handleWindowFocus(_ request: Request) async throws -> Response {
    ...
    guard let (windowElement, windowInfo) = resolveWindowElement(filter: req.window) else {
...
private func handleScreenshotWindow(_ request: Request) async throws -> Response {
    let req: ScreenshotWindowRequest = try await decode(request)
    let windows = enumerateWindows()
    guard let win = windows.first(where: { windowMatches($0, filter: req.window) }) else {
        return jsonError("No window matching '\(req.window)'")
    }
    guard let windowID = resolveCGWindowID(for: win) else {
        return jsonError("Could not resolve CGWindowID for '\(req.window)'")
    }
    guard let image = CGWindowListCreateImage(.null, .optionIncludingWindow, windowID, [.boundsIgnoreFraming]) else {
        return jsonError("Failed to capture window screenshot")
    }
    return writeScreenshot(image, to: req.output)
}

private func handleScreenshotElement(_ request: Request) async throws -> Response {
    let req: ScreenshotElementRequest = try await decode(request)
    let query = ElementQuery(role: req.role, label: req.label, window: req.window, id: req.id, index: req.index)
    let roleFilter: UnifiedRole? = query.role.map { RoleMapper.map(role: $0) }
    ...
    guard case .found(let info) = result else {
        return jsonError("Element not found for screenshot")
    }
    guard let pos = info.position, let sz = info.size else {
        return jsonError("Element has no bounds for screenshot")
    }

    let padding = CGFloat(req.padding ?? 0)
    let rect = CGRect(
        x: pos.x - padding, y: pos.y - padding,
        width: sz.width + padding * 2, height: sz.height + padding * 2
    )
    guard let image = CGWindowListCreateImage(rect, .optionOnScreenOnly, kCGNullWindowID, []) else {
        return jsonError("Failed to capture element screenshot")
    }
    return writeScreenshot(image, to: req.output)
}

private func handleScreenshotRegion(_ request: Request) async throws -> Response {
    let req: ScreenshotRegionRequest = try await decode(request)
    let rect = CGRect(x: req.x, y: req.y, width: req.width, height: req.height)
    guard let image = CGWindowListCreateImage(rect, .optionOnScreenOnly, kCGNullWindowID, []) else {
        return jsonError("Failed to capture region screenshot")
    }
    return writeScreenshot(image, to: req.output)
}

private func writeScreenshot(_ image: CGImage, to outputPath: String) -> Response {
    let bitmap = NSBitmapImageRep(cgImage: image)
    guard let pngData = bitmap.representation(using: .png, properties: [:]) else {
        return jsonError("Failed to encode PNG")
    }
    do {
        try pngData.write(to: URL(fileURLWithPath: outputPath))
        return jsonOK(ActionResponse(success: true, message: "Screenshot saved to \(outputPath)"))
    } catch {
        return jsonOK(ActionResponse(success: false, message: "Failed to write screenshot: \(error)"))
    }
}
...
private func handleExec(_ request: Request) async throws -> Response {
    struct ExecResult: Encodable { let exitCode: Int32; let stdout: String; let stderr: String }
    let req: ExecRequest = try await decode(request)
    let timeout = req.timeout ?? 30

    let process = Process()
    process.executableURL = URL(fileURLWithPath: "/bin/bash")
    process.arguments = ["-c", req.command]
    ...
    return jsonOK(ExecResult(exitCode: process.terminationStatus, stdout: stdout, stderr: stderr))
}
...
func enumerateWindows() -> [WindowInfo] {
    ...
    for app in runningApps {
        ...
        let isFrontApp = (app.processIdentifier == frontApp?.processIdentifier)
...
private func resolveCGWindowID(for info: WindowInfo) -> CGWindowID? {
    ...
    for entry in list {
        ...
        if x == info.position.x && y == info.position.y &&
           w == info.size.width && h == info.size.height {
