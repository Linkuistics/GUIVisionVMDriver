...
public struct AgentTCPClient: Sendable {
    ...
    // MARK: - Screenshots (via agent)

    public func screenshotWindow(window: String, output: String) async throws -> ActionResponse {
        struct Req: Encodable { let window: String; let output: String }
        let data = try await postJSON("/screenshot-window", body: Req(window: window, output: output))
        return try decode(ActionResponse.self, from: data)
    }
    ...
    public func screenshotElement(
        ...
        struct Req: Encodable {
            let role: String?; let label: String?; let window: String?
            let id: String?; let index: Int?; let padding: Int?; let output: String
        }
        let data = try await postJSON("/screenshot-element", body: Req(role: role, label: label, window: window, id: id, index: index, padding: padding, output: output))
        return try decode(ActionResponse.self, from: data)
    }
    ...
    public func screenshotRegion(
        ...
        struct Req: Encodable {
            let window: String; let x: Int; let y: Int; let width: Int; let height: Int; let output: String
        }
        let data = try await postJSON("/screenshot-region", body: Req(window: window, x: x, y: y, width: width, height: height, output: output))
        return try decode(ActionResponse.self, from: data)
    }
    ...
    public func exec(_ command: String, timeout: Int = 30) async throws -> ExecResult {
        struct Req: Encodable { let command: String; let timeout: Int }
        let data = try await postJSON("/exec", body: Req(command: command, timeout: timeout))
