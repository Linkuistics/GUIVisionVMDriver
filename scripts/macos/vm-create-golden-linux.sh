#!/bin/bash
# Create a golden Linux VM image with guivision-agent TCP service.
...
#   - guivision-agent Python TCP service on port 8648 (systemd user service)
...
# --- Install guivision-agent ---

echo "Installing guivision-agent..."
...
if [[ ! -d "$_AGENT_DIR/guivision_agent" ]]; then
    echo "ERROR: Agent package not found at $_AGENT_DIR/guivision_agent"
    exit 1
fi

vm_ssh "sudo mkdir -p /opt/guivision"
# tar the agent package on the host, pipe via SSH to extract in VM
tar -cf - -C "$_AGENT_DIR" guivision_agent | \
    ssh $_SSH_OPTS "$_VANILLA_USER@$_IP" "sudo tar -xf - -C /opt/guivision"

# Create a launcher script
vm_ssh "sudo tee /opt/guivision/run-agent.sh > /dev/null << 'LAUNCHER'
#!/bin/bash
cd /opt/guivision
exec python3 -m guivision_agent
LAUNCHER"
vm_ssh "sudo chmod +x /opt/guivision/run-agent.sh"

# Create systemd user service (runs in desktop session for AT-SPI2 access)
vm_ssh "mkdir -p ~/.config/systemd/user"
vm_ssh "cat > ~/.config/systemd/user/guivision-agent.service << 'UNIT'
[Unit]
Description=GUIVision Agent TCP Service
After=graphical-session.target

[Service]
Type=simple
ExecStart=/opt/guivision/run-agent.sh
Restart=always
RestartSec=5
...
vm_ssh "ln -sf ~/.config/systemd/user/guivision-agent.service ~/.config/systemd/user/default.target.wants/guivision-agent.service"
...
vm_ssh "sudo tee -a /usr/share/glib-2.0/schemas/99-guivision.gschema.override > /dev/null << 'ATSPI'
...
if ! $_AGENT_READY; then
    ...
    vm_ssh "systemctl --user status guivision-agent.service" || true
    vm_ssh "journalctl --user -u guivision-agent.service --no-pager -n 20" || true
    exit 1
fi
