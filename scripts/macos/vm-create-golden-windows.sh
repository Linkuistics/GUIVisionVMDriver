#!/bin/bash
# Create a golden Windows 11 ARM VM image with the guivision-agent TCP service.
...
# the guivision-agent binary, VirtIO drivers, and setup scripts. SetupComplete.cmd
...
#   - guivision-agent TCP service on port 8648 (starts on logon via Task Scheduler)
...
_AGENT_EXE="$_AGENT_PROJECT/bin/Release/net9.0-windows/win-arm64/publish/guivision-agent.exe"
...
if [[ ! -f "$_AGENT_EXE" ]]; then
    echo "ERROR: Agent build failed — guivision-agent.exe not found"
    exit 1
fi
...
# Copy agent binary
cp "$_AGENT_EXE" "$_AUTOUNATTEND_TMP/guivision-agent.exe"
...
echo "  Agent port forwarded: localhost:$_AGENT_PORT"
...
qemu-system-aarch64 \
    ...
    -netdev "user,id=net0,hostfwd=tcp::${_AGENT_PORT}-:${_AGENT_PORT}" \
...
echo "    3. Install guivision-agent and Chocolatey"
echo ""
